// SPDX-License-Identifier: GPL-2.0
/*
 * sc233hgs driver
 *
 * Copyright (C) 2024 Sophon Co., Ltd.
 *
 */
#include <linux/clk.h>
#include <linux/acpi.h>
#include <linux/i2c.h>
#include <linux/i2c-dev.h>
#include <linux/module.h>
#include <linux/delay.h>
#include <linux/pm_runtime.h>
#include <media/v4l2-ctrls.h>
#include <media/v4l2-device.h>
#include <media/v4l2-fwnode.h>
#include <linux/of.h>
#include <linux/of_graph.h>
#include <linux/of_gpio.h>
#include <linux/pinctrl/consumer.h>
#include <linux/of_device.h>
#include <linux/gpio/consumer.h>

#include <linux/comm_cif.h>
#include <linux/sns_v4l2_uapi.h>

#include "sc233hgs.h"

/* I2C per write of bits */
#define REG_VALUE_08BIT		1
#define REG_VALUE_16BIT		2
#define REG_VALUE_24BIT		3

/* Chip ID */
#define SC233HGS_CHIP_ID_ADDR_H		0x3107
#define SC233HGS_CHIP_ID_ADDR_L		0x3108
#define SC233HGS_CHIP_ID			0xcb61

/*Sensor type for isp middleware*/
#define SC233HGS_SNS_TYPE_SDR V4L2_SMS_SC233HGS_MASTER_MIPI_2M_30FPS_4lane_10BIT
#define SC233HGS_SNS_TYPE_WDR V4L2_SMS_SC233HGS_MIPI_2M_60FPS_10BIT_WDR2TO1

static const enum mipi_wdr_mode_e sc233hgs_wdr_mode = MIPI_WDR_MODE_VC;

volatile int sc233hgs_count = 0;
static int force_bus[MAX_SENSOR_DEVICE] = {[0 ... (MAX_SENSOR_DEVICE - 1)] = -1};
module_param_array(force_bus, int, &sc233hgs_count, 0644);

static int sc233hgs_probe_index;
static const unsigned short sc233hgs_i2c_list[] = {0x30};
static const int sc233hgs_bus_map[MAX_SENSOR_DEVICE] = {3, -1, -1, -1, -1, -1};

struct sc233hgs_reg_list {
	u32 num_of_regs;
	const struct sc233hgs_reg *regs;
};

/* Mode : resolution and related config&values */
struct sc233hgs_mode {
	u32 max_width;
	u32 max_height;
	u32 width;
	u32 height;
	u32 hts_def;
	u32 vts_def;
	u32 exp_def;
	u32 mipi_wdr_mode;
	u32 sns_type;
	char *sns_type_name;
	struct v4l2_fract max_fps;
	struct v4l2_fract wdr_max_fps;
	sns_sync_info_t sc233hgs_sync_info;
	struct sc233hgs_reg_list reg_list;
	struct sc233hgs_reg_list wdr_reg_list;
};

/* Mode configs */
static struct sc233hgs_mode supported_modes[] = {
	{
		.max_width  = 1920,
		.max_height = 1080,
		.width = 1920,
		.height = 1080,
		.exp_def = 400,
		.hts_def = 2382,
		.vts_def = 1574,
		.sns_type = V4L2_SMS_SC233HGS_MASTER_MIPI_2M_30FPS_4lane_10BIT,
		.sns_type_name  = "V4L2_SMS_SC233HGS_MASTER_MIPI_2M_30FPS_4lane_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 300000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_linear_1080p30_4lane_master_regs),
			.regs = mode_linear_1080p30_4lane_master_regs,
		},
	},
	{
		.max_width  = 1920,
		.max_height = 1080,
		.width = 1920,
		.height = 1080,
		.exp_def = 400,
		.hts_def = 2250,		// 2250  linear   2250  wdr
		.vts_def = 1481,		// 1481  linear   1481  wdr
		.sns_type = V4L2_SMS_SC233HGS_MIPI_2M_60FPS_10BIT,
		.sns_type_name  = "V4L2_SMS_SC233HGS_MIPI_2M_60FPS_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 600000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_linear_1080p60_regs),
			.regs = mode_linear_1080p60_regs,
		},
	},
	{
		.max_width  = 1920,
		.max_height = 1080,
		.width = 1920,
		.height = 1080,
		.exp_def = 400,
		.hts_def = 2250,		// 2250  linear   2250  wdr
		.vts_def = 1481,		// 1481  linear   1481  wdr
		.sns_type = V4L2_SMS_SC233HGS_MIPI_2M_60FPS_10BIT_WDR2TO1,
		.sns_type_name  = "V4L2_SMS_SC233HGS_MIPI_2M_60FPS_10BIT_WDR2TO1",
		.max_fps = {
			.numerator = 10000,
			.denominator = 600000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_wdr_1080p60_2to1_regs),
			.regs = mode_wdr_1080p60_2to1_regs,
		},
	},
};

struct sc233hgs {
	struct v4l2_subdev sd;
	struct media_pad pad;
	struct i2c_client *client;
	struct v4l2_ctrl_handler ctrl_handler;
	/* V4L2 Controls */
	struct v4l2_ctrl *vblank;
	struct v4l2_ctrl *hblank;
	struct v4l2_ctrl *exposure;
	/* Current mode */
	struct sc233hgs_mode *cur_mode;
	/* Mutex for serialized access */
	struct mutex mutex;
	/* Streaming on/off */
	bool streaming;
	/*dtsi config*/
	struct clk       *xvclk;
	struct gpio_desc *power_gpio;
	struct gpio_desc *reset_gpio;
	struct gpio_desc *pwdn_gpio;
	struct pinctrl   *pinctrl;

	unsigned int lane_num;
	unsigned int module_index;
};

#define to_sc233hgs(_sd)	container_of(_sd, struct sc233hgs, sd)

/* Read registers up to 4 at a time */
static int sc233hgs_read_reg(struct sc233hgs *sc233hgs, u16 reg, u32 len,
			    u32 *val)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	struct i2c_msg msgs[2];
	u8 *data_be_p;
	int ret;
	__be32 data_be = 0;
	__be16 reg_addr_be = cpu_to_be16(reg);

	if (len > 4)
		return -EINVAL;

	data_be_p = (u8 *)&data_be;
	/* Write register address */
	msgs[0].addr = client->addr;
	msgs[0].flags = 0;
	msgs[0].len = 2;
	msgs[0].buf = (u8 *)&reg_addr_be;

	/* Read data from register */
	msgs[1].addr = client->addr;
	msgs[1].flags = I2C_M_RD;
	msgs[1].len = len;
	msgs[1].buf = &data_be_p[4 - len];

	ret = i2c_transfer(client->adapter, msgs, ARRAY_SIZE(msgs));
	if (ret != ARRAY_SIZE(msgs))
		return -EIO;

	*val = be32_to_cpu(data_be);

	return 0;
}

/* Write registers up to 4 at a time */
static int sc233hgs_write_reg(struct sc233hgs *sc233hgs, u16 reg, u32 len,
			     u32 __val)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	int buf_i, val_i;
	u8 buf[6], *val_p;
	__be32 val;

	if (len > 4)
		return -EINVAL;

	buf[0] = reg >> 8;
	buf[1] = reg & 0xff;

	val = cpu_to_be32(__val);
	val_p = (u8 *)&val;
	buf_i = 2;
	val_i = 4 - len;

	while (val_i < 4)
		buf[buf_i++] = val_p[val_i++];

	if (i2c_master_send(client, buf, len + 2) != len + 2)
		return -EIO;

	usleep_range(100, 200);

	return 0;
}

/* Write a list of registers */
static int sc233hgs_write_regs(struct sc233hgs *sc233hgs,
			      const struct sc233hgs_reg *regs, u32 len)
{
	printk("len = %d\n", len);
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	int ret;
	u32 i;

	for (i = 0; i < len; i++) {
		ret = sc233hgs_write_reg(sc233hgs, regs[i].address, 1,
					regs[i].val);
		// printk("!!!!!! write reg 0x%4.4x. value = %d, rror=%d\n",
		// 			    regs[i].address, regs[i].val, ret);
		if (ret) {
			dev_err_ratelimited(&client->dev, "Failed to write reg 0x%4.4x. value = %d, rror=%d\n",
					    regs[i].address, regs[i].val, ret);

			return ret;
		}
	}

	return 0;
}

/* Open sub-device */
static int sc233hgs_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	struct v4l2_mbus_framefmt *try_fmt =
			v4l2_subdev_get_try_format(sd, fh->pad, 0);

	mutex_lock(&sc233hgs->mutex);

	/* Initialize try_fmt */
	try_fmt->width = sc233hgs->cur_mode->width;
	try_fmt->height = sc233hgs->cur_mode->height;
	try_fmt->code = MEDIA_BUS_FMT_SGRBG10_1X10;
	try_fmt->field = V4L2_FIELD_NONE;

	/* No crop or compose */
	mutex_unlock(&sc233hgs->mutex);

	return 0;
}

static int sc233hgs_set_ctrl(struct v4l2_ctrl *ctrl)
{
	struct sc233hgs *sc233hgs = container_of(ctrl->handler,
					       struct sc233hgs, ctrl_handler);
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);

	pm_runtime_put(&client->dev);

	return 0;
}

static const struct v4l2_ctrl_ops sc233hgs_ctrl_ops = {
	.s_ctrl = sc233hgs_set_ctrl,
};

static int g_mbus_config(struct v4l2_subdev *sd, unsigned int pad_id,
			 struct v4l2_mbus_config *config)
{
	config->type = V4L2_MBUS_CSI2_DPHY;
	return 0;
}

static int enum_mbus_code(struct v4l2_subdev *sd,
			  struct v4l2_subdev_pad_config *cfg,
			  struct v4l2_subdev_mbus_code_enum *code)
{
	/* Only one bayer order(GRBG) is supported */
	if (code->index > 0)
		return -EINVAL;

	code->code = MEDIA_BUS_FMT_SGRBG10_1X10;

	return 0;
}

static int enum_frame_interval(struct v4l2_subdev *sd,
			      struct v4l2_subdev_pad_config *cfg,
			      struct v4l2_subdev_frame_interval_enum *fie)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);

	fie->width  = sc233hgs->cur_mode->width;
	fie->height = sc233hgs->cur_mode->height;

	fie->interval.numerator   = sc233hgs->cur_mode->max_fps.numerator;
	fie->interval.denominator = sc233hgs->cur_mode->max_fps.denominator;

	return 0;
}

static int enum_frame_size(struct v4l2_subdev *sd,
			   struct v4l2_subdev_pad_config *cfg,
			   struct v4l2_subdev_frame_size_enum *fse)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);

	fse->min_width = sc233hgs->cur_mode->width;
	fse->max_width = sc233hgs->cur_mode->max_width;
	fse->min_height = sc233hgs->cur_mode->height;
	fse->max_height = sc233hgs->cur_mode->max_height;

	return 0;
}

static void update_pad_format(const struct sc233hgs_mode *mode, struct v4l2_subdev_format *fmt)
{
	fmt->format.width = mode->width;
	fmt->format.height = mode->height;
	fmt->format.code = MEDIA_BUS_FMT_SGRBG10_1X10;
	fmt->format.field = V4L2_FIELD_NONE;
}

static int get_pad_format(struct v4l2_subdev *sd,
			  struct v4l2_subdev_pad_config *cfg,
			  struct v4l2_subdev_format *fmt)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	struct v4l2_mbus_framefmt *framefmt;
	int ret = 0;

	mutex_lock(&sc233hgs->mutex);
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		framefmt = v4l2_subdev_get_try_format(sd, cfg, fmt->pad);
		fmt->format = *framefmt;
		ret = -ENOTTY;
	} else {
		update_pad_format(sc233hgs->cur_mode, fmt);
	}
	mutex_unlock(&sc233hgs->mutex);

	return ret;
}

static int set_pad_format(struct v4l2_subdev *sd,
			  struct v4l2_subdev_pad_config *cfg,
			  struct v4l2_subdev_format *fmt)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	struct sc233hgs_mode *mode;
	struct v4l2_mbus_framefmt *framefmt;

	mutex_lock(&sc233hgs->mutex);

	/* Only one raw bayer(GRBG) order is supported */
	if (fmt->format.code != MEDIA_BUS_FMT_SGRBG10_1X10)
		fmt->format.code = MEDIA_BUS_FMT_SGRBG10_1X10;

	mode = v4l2_find_nearest_size(supported_modes,
				      ARRAY_SIZE(supported_modes),
				      width, height,
				      fmt->format.width, fmt->format.height);
	update_pad_format(mode, fmt);
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		framefmt = v4l2_subdev_get_try_format(sd, cfg, fmt->pad);
		*framefmt = fmt->format;
	} else {
		sc233hgs->cur_mode = mode;
	}

	mutex_unlock(&sc233hgs->mutex);

	return 0;
}

static void sc233hgs_standby(struct sc233hgs *sc233hgs)
{
	sc233hgs_write_reg(sc233hgs, 0x3000, REG_VALUE_08BIT, 0x01);
	sc233hgs_write_reg(sc233hgs, 0x3002, REG_VALUE_08BIT, 0x01);
}

// static void sc233hgs_restart(struct sc233hgs *sc233hgs)
// {
// 	sc233hgs_write_reg(sc233hgs, 0x3000, REG_VALUE_08BIT, 0x00);
// 	sc233hgs_write_reg(sc233hgs, 0x3002, REG_VALUE_08BIT, 0x00);
// }

/* Start streaming */
static int start_streaming(struct sc233hgs *sc233hgs)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	const struct sc233hgs_reg_list *reg_list;
	const sns_sync_info_t *sync_info;
	int ret;

	reg_list = &sc233hgs->cur_mode->reg_list;

	printk("reg_list->num_of_regs = %d\n", reg_list->num_of_regs);

	ret = sc233hgs_write_regs(sc233hgs, reg_list->regs, reg_list->num_of_regs);
	if (ret) {
		dev_err(&client->dev, "%s failed to set mode\n", __func__);
		return ret;
	}
	printk("sc233hgs_write_regs success!\n");

	sync_info = &sc233hgs->cur_mode->sc233hgs_sync_info;
	printk("sync_info->num_of_regs = %d\n", sync_info->num_of_regs);

	if (sync_info->num_of_regs > 0) {
		ret = sc233hgs_write_regs(sc233hgs, (struct sc233hgs_reg *)sync_info->regs,
					sync_info->num_of_regs);
		if (ret) {
			dev_err(&client->dev, "%s failed to set default\n", __func__);
			return ret;
		}
	}
	printk("sync_info sc233hgs_write_regs success!\n");

	usleep_range(100 * 1000, 200 * 1000);
	/* Apply customized values from user */
	ret =  __v4l2_ctrl_handler_setup(sc233hgs->sd.ctrl_handler);
	if (ret)
		return ret;

	dev_info(&client->dev, "wdr_mode(%d) reg setting done\n", sc233hgs->cur_mode->mipi_wdr_mode);

	return ret;
}

/* Stop streaming */
static int stop_streaming(struct sc233hgs *sc233hgs)
{
	sc233hgs_standby(sc233hgs);
	return 0;
}

static int set_stream(struct v4l2_subdev *sd, int enable)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret = 0;

	mutex_lock(&sc233hgs->mutex);
	if (sc233hgs->streaming == enable) {
		mutex_unlock(&sc233hgs->mutex);
		return 0;
	}

	if (enable) {
		ret = pm_runtime_get_sync(&client->dev);
		if (ret < 0) {
			pm_runtime_put_noidle(&client->dev);
			goto err_unlock;
		}

		//reset sensor
		if (!IS_ERR(sc233hgs->reset_gpio)) {
			gpiod_set_value_cansleep(sc233hgs->reset_gpio, 0);
			msleep(100);
			gpiod_set_value_cansleep(sc233hgs->reset_gpio, 1);
			msleep(100);
		}

		/*
		 * Apply default & customized values
		 * and then start streaming.
		 */
		ret = start_streaming(sc233hgs);
		if (ret)
			goto err_rpm_put;
	} else {
		stop_streaming(sc233hgs);
		pm_runtime_put(&client->dev);
	}

	sc233hgs->streaming = enable;
	mutex_unlock(&sc233hgs->mutex);

	dev_info(&client->dev, "set stream(%d) success\n", enable);

	return ret;

err_rpm_put:
	pm_runtime_put(&client->dev);
err_unlock:
	mutex_unlock(&sc233hgs->mutex);

	return ret;
}

static int __maybe_unused suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);

	if (sc233hgs->streaming)
		stop_streaming(sc233hgs);

	return 0;
}

static int __maybe_unused resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	int ret;

	if (sc233hgs->streaming) {
		ret = start_streaming(sc233hgs);
		if (ret)
			goto error;
	}

	return 0;

error:
	stop_streaming(sc233hgs);
	sc233hgs->streaming = false;
	return ret;
}

/* Verify chip ID */
static int sc233hgs_identify_module(struct sc233hgs *sc233hgs)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	int ret;
	int val1, val2;
	int read_data = 0;

	sc233hgs_write_reg(sc233hgs, 0x3000, REG_VALUE_08BIT, 0x01);/* STANDBY */
	sc233hgs_write_reg(sc233hgs, 0x3002, REG_VALUE_08BIT, 0x01);/* XTMSTA */

	usleep_range(20 * 1000, 20 * 1000);

	ret = sc233hgs_read_reg(sc233hgs, SC233HGS_CHIP_ID_ADDR_H,
			       REG_VALUE_08BIT, &val1);

	dev_info(&client->dev, "read id:0x%x, ret:%d", val1, ret);

	if (ret)
		return ret;

	ret = sc233hgs_read_reg(sc233hgs, SC233HGS_CHIP_ID_ADDR_L,
			       REG_VALUE_08BIT, &val2);

	read_data = ((val1 & 0xFF) << 8) | (val2 & 0xFF);

	//if (read_data != SC233HGS_CHIP_ID) {
	//	dev_err(&client->dev, "chip id(%x) mismatch, read(%x)\n",
	//		SC233HGS_CHIP_ID, read_data);
	//	return -EIO;
	//}

	return 0;
}

static void sc233hgs_mirror_flip(struct sc233hgs *sc233hgs, int orient)
{
	u8 val = 0;

	switch (orient) {
	case 0:
		break;
	case 1:
		val |= 0x6;
		break;
	case 2:
		val |= 0x60;
		break;
	case 3:
		val |= 0x66;
		break;
	default:
		return;
	}

	sc233hgs_write_reg(sc233hgs, 0x3221, REG_VALUE_08BIT, val);
}

static int sc233hgs_update_link_menu(struct sc233hgs *sc233hgs)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	struct v4l2_ctrl_handler *ctrl_hdlr;
	struct v4l2_ctrl *ctrl;
	int wdr_index = SNS_CFG_TYPE_WDR_MODE;
	int id = sc233hgs->module_index;
	int ret;
	int i;

	sc233hgs_link_cif_menu[id][wdr_index] = sc233hgs->cur_mode->mipi_wdr_mode;

	dev_info(&client->dev, "update mipi_mode:%lld", sc233hgs_link_cif_menu[id][wdr_index]);

	ctrl_hdlr = sc233hgs->sd.ctrl_handler;
	v4l2_ctrl_handler_free(ctrl_hdlr);

	ret = v4l2_ctrl_handler_init(ctrl_hdlr, 10);
	if (ret) {
		dev_err(&client->dev, "%s ctrl handler init failed (%d)\n",
			__func__, ret);
		return ret;
	}

	for (i = 0; i < SNS_CFG_TYPE_MAX; i++) {
		ctrl = v4l2_ctrl_new_int_menu(ctrl_hdlr, &sc233hgs_ctrl_ops, V4L2_CID_LINK_FREQ,
					      sc233hgs_link_cif_menu[id][i], 0,
					       (const s64 *)sc233hgs_link_cif_menu[id]);

		if (ctrl)
			ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;
	}

	if (ctrl_hdlr->error) {
		ret = ctrl_hdlr->error;
		dev_err(&client->dev, "%s new int menu failed (%d)\n",
			__func__, ret);
		goto error;
	}

	return 0;

error:
	v4l2_ctrl_handler_free(ctrl_hdlr);

	return ret;
}

static long sc233hgs_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);
	long ret = 0;

	switch (cmd) {
	case SNS_V4L2_GET_TYPE:
	{
		int type = 0;

		if (sc233hgs->cur_mode->mipi_wdr_mode == MIPI_WDR_MODE_NONE) {//linear
			type = SC233HGS_SNS_TYPE_SDR;
		} else {//wdr
			type = SC233HGS_SNS_TYPE_WDR;
		}
		memcpy(arg, &type, sizeof(int));
		break;
	}

	case SNS_V4L2_SET_MIRROR_FLIP:
	{
		int orient = 0;

		memcpy(&orient, arg, sizeof(int));
		sc233hgs_mirror_flip(sc233hgs, orient);
		break;
	}

	case SNS_V4L2_GET_I2C_INFO:
	{
		struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
		sns_i2c_info_t i2c_info;

		i2c_info.i2c_addr =  client->addr;
		i2c_info.i2c_idx  =  client->adapter->i2c_idx;
		memcpy(arg, &i2c_info, sizeof(sns_i2c_info_t));
		break;
	}

#ifdef SC233HGS_WDR
	case SNS_V4L2_SET_HDR_ON:
	{
		int hdr_on = 0;

		memcpy(&hdr_on, arg, sizeof(int));

		if (hdr_on)
			memcpy(sc233hgs->cur_mode, &supported_modes[1], sizeof(struct sc233hgs_mode));
		else
			memcpy(sc233hgs->cur_mode, &supported_modes[0], sizeof(struct sc233hgs_mode));

		sc233hgs_update_link_menu(sc233hgs);
		break;
	}
#endif

	case SNS_V4L2_SET_SNS_SYNC_INFO:
	{
		memcpy(&sc233hgs->cur_mode->sc233hgs_sync_info, arg, sizeof(sns_sync_info_t));
		break;
	}

	default:
		ret = -ENOIOCTLCMD;
		break;
	}

	return ret;
}

#ifdef CONFIG_COMPAT
static long sc233hgs_compat_ioctl32(struct v4l2_subdev *sd,
				   unsigned int cmd, unsigned long arg)
{
	long ret;

	switch (cmd) {
	default:
		ret = -ENOIOCTLCMD;
		break;
	}

	return ret;
}
#endif

static const struct v4l2_subdev_core_ops sc233hgs_core_ops = {
	.ioctl = sc233hgs_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = sc233hgs_compat_ioctl32,
#endif
};

static const struct v4l2_subdev_video_ops sc233hgs_video_ops = {
	.s_stream = set_stream,
};

static const struct v4l2_subdev_pad_ops sc233hgs_pad_ops = {
	.enum_mbus_code = enum_mbus_code,
	.get_fmt = get_pad_format,
	.set_fmt = set_pad_format,
	.enum_frame_size = enum_frame_size,
	.enum_frame_interval = enum_frame_interval,
	.get_mbus_config = g_mbus_config,
};

static const struct v4l2_subdev_ops sc233hgs_subdev_ops = {
	.core	= &sc233hgs_core_ops,
	.video  = &sc233hgs_video_ops,
	.pad    = &sc233hgs_pad_ops,
};

static const struct media_entity_operations sc233hgs_subdev_entity_ops = {
	.link_validate = v4l2_subdev_link_validate,
};

static const struct v4l2_subdev_internal_ops sc233hgs_internal_ops = {
	.open = sc233hgs_open,
};

/* Initialize control handlers */
static int sc233hgs_init_controls(struct sc233hgs *sc233hgs, int index_id)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc233hgs->sd);
	struct v4l2_fwnode_device_properties props;
	struct v4l2_ctrl_handler *ctrl_hdlr;
	struct v4l2_ctrl *ctrl;
	int ret;
	int i;

	ctrl_hdlr = &sc233hgs->ctrl_handler;
	ret = v4l2_ctrl_handler_init(ctrl_hdlr, 10);
	if (ret) {
		dev_err(&client->dev, "%s ctrl handler init failed (%d)\n",
			__func__, ret);
		return ret;
	}

	mutex_init(&sc233hgs->mutex);
	ctrl_hdlr->lock = &sc233hgs->mutex;
	for (i = 0; i < SNS_CFG_TYPE_MAX; i++) {
		if (i == SNS_CFG_TYPE_WDR_MODE)
			sc233hgs->cur_mode->mipi_wdr_mode = sc233hgs_link_cif_menu[index_id][i];

		ctrl = v4l2_ctrl_new_int_menu(ctrl_hdlr, &sc233hgs_ctrl_ops, V4L2_CID_LINK_FREQ,
					      sc233hgs_link_cif_menu[index_id][i], 0,
					       (const s64 *)sc233hgs_link_cif_menu[index_id]);

		if (ctrl)
			ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;
	}

	if (ctrl_hdlr->error) {
		ret = ctrl_hdlr->error;
		dev_err(&client->dev, "%s new std menu failed (%d)\n",
			__func__, ret);
		goto error;
	}

	ret = v4l2_fwnode_device_parse(&client->dev, &props);
	if (ret)
		goto error;

	ret = v4l2_ctrl_new_fwnode_properties(ctrl_hdlr, &sc233hgs_ctrl_ops,
					      &props);
	if (ret)
		goto error;

	sc233hgs->sd.ctrl_handler = ctrl_hdlr;

	return 0;

error:
	v4l2_ctrl_handler_free(ctrl_hdlr);
	mutex_destroy(&sc233hgs->mutex);

	return ret;
}

static void sc233hgs_free_controls(struct sc233hgs *sc233hgs)
{
	v4l2_ctrl_handler_free(sc233hgs->sd.ctrl_handler);
	mutex_destroy(&sc233hgs->mutex);
}

static int sc233hgs_probe(struct i2c_client *client,
			 const struct i2c_device_id *devid)
{
	struct sc233hgs *sc233hgs;
	struct v4l2_subdev *sd;
	struct device *dev = &client->dev;
	int index_id = sc233hgs_probe_index;
	int addr_num = sizeof(sc233hgs_i2c_list) / sizeof(unsigned short);
	u32 bus_id, i2c_addr, use_defualt = 1;
	int ret = -1;
	int i;

	dev_info(dev, "probe id[%d] start\n", sc233hgs_probe_index);

	sc233hgs_probe_index++;

	if (index_id >= MAX_SENSOR_DEVICE || index_id < 0) {
		dev_info(dev, "invalid devid(%d)\n", index_id);
		return ret;
	}

	sc233hgs = devm_kzalloc(&client->dev, sizeof(*sc233hgs), GFP_KERNEL);
	if (!sc233hgs)
		return -ENOMEM;

	sd = &sc233hgs->sd;

	if (!of_property_read_u32(client->dev.of_node,"reg-addr", &i2c_addr) &&
		!of_property_read_u32(client->dev.of_node,"bus-id", &bus_id) &&
		!sc233hgs_count) {
		printk("sc233hgs_probe reg = %x\n", i2c_addr);
		printk("sc233hgs_probe bus-id = %x\n", bus_id);
		client->addr = i2c_addr;
		client->adapter = i2c_get_adapter(bus_id);
		sc233hgs->client = client;
		v4l2_i2c_subdev_init(sd, client, &sc233hgs_subdev_ops);
		/* Check module identity */
		ret = sc233hgs_identify_module(sc233hgs);
		if (ret) {
			dev_info(dev, "id[%d] bus[%d] i2c_addr[%d][0x%x] no sensor found,use default\n",
				index_id, bus_id, i, client->addr);
			use_defualt = 1;
		} else {
			dev_info(dev, "id[%d] bus[%d] i2c_addr[0x%x] sensor found\n",
				 index_id, bus_id, client->addr);
			use_defualt = 0;
		}
	}

	for (i = 0; i < addr_num && use_defualt; i++) {
		if (force_bus[index_id] < 0)
			bus_id = sc233hgs_bus_map[index_id];
		else
			bus_id = force_bus[index_id];

		if (bus_id < 0 || bus_id > MAX_I2C_BUS_NUM)
			return ret;

		client->addr = sc233hgs_i2c_list[i];
		client->adapter = i2c_get_adapter(bus_id);
		sc233hgs->client = client;
		v4l2_i2c_subdev_init(sd, client, &sc233hgs_subdev_ops);

		/* Check module identity */
		ret = sc233hgs_identify_module(sc233hgs);
		if (ret) {
			dev_info(dev, "id[%d] bus[%d] i2c_addr[%d][0x%x] no sensor found\n",
				 index_id, bus_id, i, client->addr);

			if (i == addr_num - 1)
				return ret;

			continue;
		} else {
			dev_info(dev, "id[%d] bus[%d] i2c_addr[0x%x] sensor found\n",
				 index_id, bus_id, client->addr);
			break;
		}
	}

	sc233hgs->module_index = index_id;

	sc233hgs->cur_mode = devm_kzalloc(&client->dev,
						sizeof(struct sc233hgs_mode), GFP_KERNEL);
	memcpy(sc233hgs->cur_mode, &supported_modes[0], sizeof(struct sc233hgs_mode));

	memset(&sc233hgs->cur_mode->sc233hgs_sync_info, 0, sizeof(sns_sync_info_t));

	mutex_init(&sc233hgs->mutex);

	ret = sc233hgs_init_controls(sc233hgs, index_id);
	if (ret)
		return ret;

	/* Initialize subdev */
	sd->internal_ops = &sc233hgs_internal_ops;
	sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	sd->entity.ops = &sc233hgs_subdev_entity_ops;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;

	/* Initialize source pad */
	sc233hgs->pad.flags = MEDIA_PAD_FL_SOURCE;
	ret = media_entity_pads_init(&sd->entity, 1, &sc233hgs->pad);
	if (ret) {
		dev_err(&client->dev, "failed to init pads:%d\n", ret);
		goto error_handler_free;
	}

	snprintf(sd->name, sizeof(sd->name), "cam%d_%s %s",
		 sc233hgs->module_index, "sc233hgs", dev_name(sd->dev));

	ret = v4l2_async_register_subdev_sensor_common(sd);
	if (ret < 0) {
		dev_err(&client->dev, "failed to async subdev:%d\n", ret);
		goto error_media_entity;
	}

	/*
	 * Device is already turned on by i2c-core with ACPI domain PM.
	 * Enable runtime PM and turn off the device.
	 */
	pm_runtime_set_active(&client->dev);
	pm_runtime_enable(&client->dev);
	pm_runtime_idle(&client->dev);

	dev_info(dev, "sensor_%d probe success\n", index_id);

	return 0;

error_media_entity:
	media_entity_cleanup(&sc233hgs->sd.entity);

error_handler_free:
	sc233hgs_free_controls(sc233hgs);
	dev_err(&client->dev, "%s failed:%d\n", __func__, ret);

	return ret;
}

static int sc233hgs_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc233hgs *sc233hgs = to_sc233hgs(sd);

	sc233hgs_probe_index = 0;

	v4l2_async_unregister_subdev(sd);
	media_entity_cleanup(&sd->entity);
	sc233hgs_free_controls(sc233hgs);

	pm_runtime_disable(&client->dev);

	return 0;
}

static const struct of_device_id sc233hgs_of_match[] = {
	{ .compatible = "v4l2,sensor0" },
	{ .compatible = "v4l2,sensor1" },
	{ .compatible = "v4l2,sensor2" },
	{ .compatible = "v4l2,sensor3" },
	{ .compatible = "v4l2,sensor4" },
	{ .compatible = "v4l2,sensor5" },
	{},
};
MODULE_DEVICE_TABLE(of, sc233hgs_of_match);

static const struct dev_pm_ops sc233hgs_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(suspend, resume)
};

static struct i2c_driver sc233hgs_i2c_driver = {
	.driver = {
		.name = "sc233hgs",
		.pm = &sc233hgs_pm_ops,
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(sc233hgs_of_match),
	},
	.probe    = sc233hgs_probe,
	.remove   = sc233hgs_remove,
};

static int __init sensor_mod_init(void)
{
	pr_info("== sc233hgs mod add ==\n");

	return i2c_add_driver(&sc233hgs_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&sc233hgs_i2c_driver);
}

module_init(sensor_mod_init);
module_exit(sensor_mod_exit);

MODULE_DESCRIPTION("sc233hgs sensor driver");
MODULE_LICENSE("GPL v2");
